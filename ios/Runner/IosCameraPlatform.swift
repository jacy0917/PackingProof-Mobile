...
class IosCameraPlatform: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate, AVCaptureMetadataOutputObjectsDelegate {
    ...
    func splitWork(
        ...
        let signpostID = OSSignpostID(log: Self.performanceLog)
        os_signpost(.begin, log: Self.performanceLog, name: "CameraRecordingSplit", signpostID: signpostID)
        
        sessionQueue.async { [weak self] in
            guard let self = self, !self.isDisposed else {
                if let self = self {
                    self.finishPerformanceOperation(timing, signpostID: signpostID, signpostName: "CameraRecordingSplit", succeeded: false)
                } else {
                ...
                completion: { [weak self] result in
                    ...
                    case .success(let request):
                        ...
                        self.finishWriter(timing: timing) { writerResult in
                            switch writerResult {
                            case .success(let stopDto):
                                self.eventApi.segmentEnded(stopDto: stopDto, completion: { _ in })
                                do {
