...
class IosCameraEventApiImplementation {
    ...
    // ⭐ 使用 Any 避免与 Pigeon 生成的 CameraRecordingStopDto 冲突
    func segmentEnded(stopDto: Any, completion: @escaping (Result<Void, Error>) -> Void) {
        completion(.success(()))
    }
    // ⭐ 使用 Any 避免与 Pigeon 生成的 BarcodeCandidateDto 冲突
...
// ⭐ ========== PigeonPlatform.swift 需要的类型（Pigeon 未生成） ==========
...
// ⭐ IosCameraHostApiSetup 是 Pigeon 生成的类，不需要在这里定义
// ⭐ IosCameraHostApi 由 Pigeon 生成协议，不需要在这里定义
// ⭐ CameraRecordingStopDto 由 Pigeon 生成，不需要在这里定义
// ⭐ BarcodeCandidateDto 由 Pigeon 生成，不需要在这里定义
